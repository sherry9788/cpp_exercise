...
{
	vector<int> f, s;
	fstream f_in("data.txt", fstream::in);
	vector<int> tmp;
	int int_tmp;
	while (f_in >> int_tmp)
	...
	for (unsigned i = 0; i < tmp.size() / 2; ++i)
		f.push_back(tmp[i]);
	for (unsigned i = tmp.size() / 2; i < tmp.size(); ++i)
		s.push_back(tmp[i]);
	bintree<int> *b = create_bintree(f, s);
	b->RootThi(print);
	cerr << endl;
	cerr << "The depth of the tree is " << b->depth() << endl;
	cerr << "The number of the descendant of the tree is "
		<< b->n_descendant() << endl;
	return 0;
}
