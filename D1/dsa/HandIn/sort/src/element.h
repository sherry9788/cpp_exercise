...
template <typename K, typename V>
void element<K, V>::bubbleSort(int &n_compare) {
	n_compare = 0;
	auto ptr = this;
	// b 用来记录是否已经有序
	bool b = false;
...
template <typename K, typename V>
void element<K, V>::insertsort(int &n_compare) {
	// 如果只有一个元素，返回
	if (!next)
		return;
	n_compare = 1;
...
template <typename K, typename V>
void element<K, V>::select_sort(int &n_compare) {
	n_compare = 0;
	// 用 tmp 记录当前操作的元素
	auto tmp = this;
	while (tmp) {
...
template <typename K, typename V>
void element<K, V>::heap_sort(int &n_compare) {
	n_compare = 0;
	auto ptr = this;
	int n = 1;
	// 统计元素个数
