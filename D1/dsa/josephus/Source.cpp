...
void JosephLoop(int num, int step, int ini = 1)
{//num 为总人数， step 为步长， ini 是开始计数的点
	link<int> *Loop = new link<int>();
	Loop->create_loop(num);
	Loop = (*Loop)[ini];
	Loop->SetInfo();
	for (int i = 0; i < num; ++i)
	{
		...
		cerr << Loop->info << endl;
#endif // PRINT
		Loop = Loop->deleteLink();
	}
	delete Loop;
}

void JosephLoop4seq(int num, int step, int ini = 1)
{
	seq<int> *Seq = new seq<int>();
	Seq->createSeq(num);
	for (int i = ini + step - 2; Seq->n != 0; i = (i + step - 1) % Seq->n)
	{
#ifdef PRINT
